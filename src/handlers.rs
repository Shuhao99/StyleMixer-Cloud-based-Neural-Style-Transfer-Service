...
#[post("/upload")]
pub async fn upload(mut payload: Multipart, client: web::Data<Client>) -> Result<HttpResponse> {
    ...
    if let (Some(file_path), Some(style_id)) = (file_path, style_id) {
        ...
        match response {
            Ok(neural_style_response) => Ok(HttpResponse::Ok().json(neural_style_response)),
...
async fn call_neural_style_api(
    ...
    let form = reqwest::multipart::Form::new()
        ...
        .text(
            "api_key",
            "VCGKSUTDMUCYMOHTEPAYYBGGKTCWBXCQGCTNGPIADSKRSRYJ",
        )
